            ...
            if (bakery instanceof ILayeredBlockBakery) {
                ...
                for (RenderType renderType : RenderType.getBlockRenderTypes()) {
                    if (RenderTypeLookup.canRenderInLayer(state, renderType)) {
                        ...
                        quads.addAll(layeredBakery.bakeLayerFace(null, renderType, state, data));
                        layerGeneralQuads.put(renderType, quads);
                    }
                }
                ...
                for (RenderType renderType : RenderType.getBlockRenderTypes()) {
                    if (RenderTypeLookup.canRenderInLayer(state, renderType)) {
                        ...
                        for (Direction face : Direction.values()) {
                            List<BakedQuad> quads = new LinkedList<>();
                            quads.addAll(layeredBakery.bakeLayerFace(face, renderType, state, data));
                            faceQuadMap.put(face, quads);
                        }
                        layerFaceQuadMap.put(renderType, faceQuadMap);
                    }
                }
                ModelProperties properties = new ModelProperties(true, true, null);
