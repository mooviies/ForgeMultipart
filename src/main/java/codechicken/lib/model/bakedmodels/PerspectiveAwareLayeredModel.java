...
public class PerspectiveAwareLayeredModel extends AbstractPerspectiveLayeredModel {
    ...
    public PerspectiveAwareLayeredModel(Map<RenderType, Map<Direction, List<BakedQuad>>> layerFaceQuadMap, Map<RenderType, List<BakedQuad>> layerGeneralQuads, PerspectiveProperties properties, RenderType generalRenderType) {
        super(properties, generalRenderType);
        this.layerFaceQuadMap = ImmutableMap.copyOf(layerFaceQuadMap);
        this.layerGeneralQuads = ImmutableMap.copyOf(layerGeneralQuads);
    }

    @Override
    public List<BakedQuad> getLayerQuads(BlockState state, Direction side, RenderType renderType, Random rand, IModelData data) {
        if (side == null) {
            if (layerGeneralQuads.containsKey(renderType)) {
                return layerGeneralQuads.get(renderType);
            }
        } else if (layerFaceQuadMap.containsKey(renderType)) {
            Map<Direction, List<BakedQuad>> faceQuadMap = layerFaceQuadMap.get(renderType);
