...
public class CCBlockRendererDispatcher extends BlockRendererDispatcher {
    ...
    @Override
    public void renderBlockDamage(BlockState state, BlockPos pos, TextureAtlasSprite sprite, ILightReader world) {
    ...
    @Override
    public boolean renderBlock(BlockState state, BlockPos pos, ILightReader world, BufferBuilder buffer, Random random, IModelData modelData) {
    ...
    public boolean renderFluid(BlockPos pos, ILightReader world, BufferBuilder buffer, IFluidState state) {
        Optional<ICCBlockRenderer> renderOpt = BlockRenderingRegistry.getBlockRenderers().stream().filter(e -> e.canHandle(world, pos, state)).findFirst();
        if (renderOpt.isPresent()) {
